...
fn get_head_tree(repo: &Path, sha1: &str) -> BTreeMap<String, String> {
    ...
    if let Ok((obj_type, content)) = storage::read_object(repo, sha1) {
        if obj_type == "commit" {
            if let Ok(commit) =
                Commit::deserialize(&agit_core::objects::format_object_data("commit", &content))
            {
...
fn get_modified_changes(repo: &Path, index: &Index) -> Vec<String> {
    ...
    for (path, entry) in &index.entries {
        ...
        if let Ok(content) = fs::read(&full_path) {
            let blob = agit_core::objects::blob::Blob::new(content);
...
fn list_untracked(repo: &Path, index: &Index) {
    ...
    let matcher = agit_core::ignore::IgnoreMatcher::load(repo, Path::new(""));
...
fn collect_untracked(
    ...
    current: &Path,
    index: &Index,
    matcher: &agit_core::ignore::IgnoreMatcher,
