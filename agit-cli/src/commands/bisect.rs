...
use agit_core::bisect::{self, BisectState};
use agit_core::checkout;
use agit_core::repo;
...
fn bisect_good(
    ...
    let sha = match rev {
        ...
        None => agit_core::refs::read_head(repo)?,
    };
...
fn bisect_reset(repo: &std::path::Path) -> Result<(), Box<dyn std::error::Error>> {
    ...
    if !state.original_head.is_empty() {
        ...
        {
            // 分离 HEAD
            agit_core::refs::write_head(repo, &state.original_head)?;
        } else if let Some(branch) = state.original_head.strip_prefix("ref: refs/heads/") {
            // 符号引用
            agit_core::refs::write_head(repo, &format!("ref: refs/heads/{}", branch))?;
        } else {
            agit_core::refs::write_head(repo, &state.original_head)?;
        }
...
fn write_head_detached(
    ...
    agit_core::refs::write_head(repo, sha)?;
    Ok(())
}
