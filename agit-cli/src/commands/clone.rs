...
use agit_core::protocol::create_transport;
use agit_core::refs;
...
pub fn run(url: &str) -> Result<(), Box<dyn std::error::Error>> {
    ...
    let objects = transport.fetch_objects(std::slice::from_ref(&head_ref), &[])?;
    ...
    let (_, tree_content) = storage::read_object(
        &repo_dir,
        &agit_core::remote_utils::resolve_commit_to_tree(&repo_dir, &head_ref)?,
    )?;
    let tree_data = agit_core::objects::format_object_data("tree", &tree_content);
...
fn build_index_from_tree(
    repo: &Path,
    tree: &agit_core::objects::tree::Tree,
    prefix: &str,
...
fn add_tree_to_index(
    ...
    for entry in &tree.entries {
        ...
        if entry.mode == "40000" {
            ...
            let sub_data = agit_core::objects::format_object_data("tree", &sub_content);
            let sub_tree = agit_core::objects::tree::Tree::deserialize(&sub_data)?;
