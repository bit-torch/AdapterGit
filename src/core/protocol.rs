...
pub fn parse_packfile(data: &[u8]) -> Result<ObjectList, Box<dyn std::error::Error>> {
    ...
    for _ in 0..num_objects {
        ...
        match obj_type {
            1..=4 => {
                let decompressed = decompress_zlib(&data[pos..])?;
                content = decompressed;
                pos += find_zlib_end(&data[pos..]);
            }
            6 => {
                pos += size_of_varint(&data[pos..]);
                let decompressed = decompress_zlib(&data[pos..])?;
                content = decompressed;
                pos += find_zlib_end(&data[pos..]);
            }
        ...
        let mut obj_data = Vec::with_capacity(header.len() + content.len());
        obj_data.extend_from_slice(header.as_bytes());
        obj_data.extend_from_slice(&content);

        let sha1 = crate::core::hash::hash_bytes(&obj_data);
        objects.push((sha1, obj_data));
    }
...
fn decompress_zlib(data: &[u8]) -> Result<Vec<u8>, Box<dyn std::error::Error>> {
    crate::core::compression::decompress(data)
}

fn find_zlib_end(data: &[u8]) -> usize {
    let mut pos = 0;
    while pos + 1 < data.len() {
        if is_zlib_end(&data[pos..]) {
            return pos + 1;
        }
        pos += 1;
    }
...
fn size_of_varint(data: &[u8]) -> usize {
    let mut pos = 0;
